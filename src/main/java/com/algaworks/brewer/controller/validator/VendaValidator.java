...
public class VendaValidator implements Validator {
	...
	public void validate(Object target, Errors errors) {
		ValidationUtils.rejectIfEmpty(errors, "cliente.codigo", "", "Seleciona um cliente na pesquisa rápida");
		
		Venda venda = (Venda) target;
		
		validarSeInformouApenasHoraDeEntrega(errors, venda);
	...
	private void validarValorTotalNegativo(Errors errors, Venda venda) {
		if (venda.getValorTotal().compareTo(BigDecimal.ZERO) < 0) {
			errors.reject("", "Valor toal não pode ser negativo");
		}
	}
	...
	private void validarSeInformouApenasHoraDeEntrega(Errors errors, Venda venda) {
		if (venda.getDataHoraEntrega() != null && venda.getDataEntrega() == null) {
